...
export default function AdminDeposits() {
  ...
  async function load() {
    const r = await fetch(`/api/admin/deposits?status=${tab}`, { cache: 'no-store' });
  ...
  async function act(id: string, action: 'APPROVE' | 'REJECT') {
    ...
    const r = await fetch('/api/admin/deposits', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id, action, note }) });
          ...
          <tbody>
            ...
            {items.map((d) => (
              <tr key={d.id} className="border-t border-line/60">
                ...
                <td className="px-4 py-3 mono text-xs truncate max-w-[160px]">{d.txHash || '—'}</td>
